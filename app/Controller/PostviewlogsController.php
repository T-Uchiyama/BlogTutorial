<?php
    ...
    class PostviewlogsController extends AppController
    {
        ...
        public function callShell()
        {
             ...
             *
             *    ・総評
             *    今回の場合はViewにそのままSetでもよかった。
             *    →理由としては今回は人気の記事と記事が一対になるようなDBで作成しているため
             *     JSONを使用すると呼び出しが二度手間になってしまうため。
             *     しかし他サイトだと巨大なDBが一つ存在し、APIを介してデータをやりとりしている
             *     のでJSONを使用して作成するのも間違いではないとのこと。
             */
            /* jsonにて連想配列を取得 */
